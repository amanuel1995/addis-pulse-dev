...
export function LanguageSwitcher({
  ...
  return (
    <nav aria-label="Campaign language" className="flex rounded-full border border-[var(--passenger-line)] bg-white p-1 text-sm font-semibold">
      {(["en", "am"] as const).map((option) => (
        <Link
          key={option}
          href={`/d/${encodeURIComponent(qrToken)}?lang=${option}`}
          aria-current={locale === option ? "page" : undefined}
          className={`min-h-9 rounded-full px-3 py-2 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--passenger-primary)] ${
            locale === option
              ? "bg-[var(--passenger-primary)] text-white"
              : "text-[var(--passenger-muted)] hover:text-[var(--passenger-primary)]"
          }`}
        >
          {option === "en" ? "EN" : "አማ"}
        </Link>
...
export function BrandMark({ name, logoUrl }: { name: string; logoUrl?: string | null }) {
  return logoUrl ? (
    <Image src={logoUrl} alt={`${name} logo`} width={48} height={48} unoptimized className="size-12 rounded-2xl border border-[var(--passenger-line)] bg-white object-contain p-1.5" />
  ) : (
    <div aria-hidden="true" className="grid size-12 place-items-center rounded-2xl bg-[var(--passenger-primary)] text-lg font-black text-white shadow-[0_10px_28px_rgba(119,28,15,0.2)]">
