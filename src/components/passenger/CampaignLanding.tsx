  ...
  return <PassengerShell><div lang={campaign.locale} dir={campaign.locale==="ar"?"rtl":"ltr"} style={{"--passenger-primary":"#8d2114"} as React.CSSProperties}>
    ...
    <main className="mx-auto w-full max-w-5xl px-4 pb-10 pt-4 sm:px-6 sm:pt-6">
      <section className="overflow-hidden rounded-[28px] border border-[var(--passenger-line)] bg-white shadow-[0_18px_55px_rgba(82,36,24,.1)]">
        <div className="flex items-center gap-4 p-5 sm:p-7"><BrandMark name={company.name} logoUrl={company.logoUrl}/><div><p className="text-xs font-black uppercase tracking-[.16em] text-[#771c0f]">{t.welcomeEyebrow}</p><h1 className="mt-1 text-2xl font-black tracking-[-.03em] sm:text-4xl">{company.name}</h1><p className="mt-1 text-sm text-[var(--passenger-muted)]">{campaign.name}</p></div></div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1.05fr_.95fr] lg:items-start">
        ...
        <aside id="lead-form" className="scroll-mt-24 lg:sticky lg:top-24"><div className="rounded-[28px] border border-[var(--passenger-line)] bg-white p-5 shadow-[0_20px_60px_rgba(82,36,24,.11)] sm:p-7"><p className="text-xs font-black uppercase tracking-[.15em] text-[var(--passenger-primary)]">{t.request}</p><h2 className="mt-2 text-2xl font-black">{t.interested}</h2><p className="mt-2 text-sm leading-6 text-[var(--passenger-muted)]">{t.formIntro(company.name)}</p><div className="mt-6"><LeadForm qrToken={qrToken} callToAction={campaign.content.callToAction} privacyNotice={campaign.content.privacyNotice||undefined} privacyNoticeVersion={campaign.content.privacyNoticeVersion} services={campaign.interests.map(i=>i.label)} companyName={company.name} locale={campaign.locale}/></div><p className="mt-5 border-t border-[var(--passenger-line)] pt-4 text-sm leading-6 text-[var(--passenger-muted)]">{t.submitDisclaimer}</p><p className="mt-3 text-xs text-[var(--passenger-muted)]">{t.assigned(campaign.driverName)}</p></div></aside>
      </div>
