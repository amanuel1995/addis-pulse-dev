...
export function LeadForm({
  ...
  return (
    <form onSubmit={submit} className="space-y-5" noValidate>
      ...
      <label className="block text-sm font-bold">Phone number <span aria-hidden="true" className="text-[var(--passenger-primary)]">*</span><input name="phone" value={values.phone} onChange={(event) => update("phone", event.target.value)} onBlur={validatePhone} type="tel" inputMode="tel" autoComplete="tel" placeholder="09... / 07..." required aria-invalid={Boolean(phoneError)} aria-describedby="phone-help phone-error" className={inputClass} /><span id="phone-help" className="mt-2 block text-sm font-normal text-[var(--passenger-muted)]">Ethiopian mobile format, for example 0911 234 567.</span>{phoneError && <span id="phone-error" role="alert" className="mt-2 block text-sm font-semibold text-red-700">{phoneError}</span>}</label>
